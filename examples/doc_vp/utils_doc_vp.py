...
class InputExampleDocVP(object):
    ...
    def __init__(self, guid, tokens, doc_vocab=None):
        ...
        Args:
            guid: Unique id for the example.
            tokens: a list. The untokenized text of the first sequence. For single
            sequence tasks, only this sequence must be specified.
            doc_vocab: a list. The label of the example. This should be
            specified for train and dev examples, but not for test examples.
        """
        self.guid = guid
        self.tokens = tokens
...
class DocvpProcessor(DataProcessor):
...
def convert_vp_examples_to_features(examples, max_seq_length,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        label_id = tokenizer.convert_tokens_to_ids(example.doc_vocab)
